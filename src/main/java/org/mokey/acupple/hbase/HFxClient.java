...
public class HFxClient {
    ...
    public void creqteTable(Class<? extends HBase> clazz){
    ...
    public HBase get(byte[] rowkey, Class<? extends HBase> clazz){
        if(rowkey == null)
            return null;
        TableMeta tableMeta = getTableMeta(clazz);
    ...
    public List<HBase> search(List<byte[]> rowkeys, FilterBase filter, Class<? extends HBase> clazz){
        TableMeta tableMeta = getTableMeta(clazz);
        ...
        List<HBase> hBaseList = Lists.newArrayList();
        try {
            Table table = connection.getTable(tableMeta.getHtableName());
